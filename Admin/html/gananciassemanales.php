...
// Consulta para las ganancias semanales
$query = "SELECT YEAR(fechaReserva) AS year, WEEK(fechaReserva, 1) AS week, SUM(servicios.precio) AS total_semana
          FROM reservas
          JOIN servicios ON reservas.servicio = servicios.id
          where reservas.id_peluqueria = $id_peluqueria
          GROUP BY YEAR(fechaReserva), WEEK(fechaReserva, 1)
          ORDER BY year, week";
$result = mysqli_query($conn, $query);
...
while ($row = mysqli_fetch_assoc($result)) {
    $semanas[] = "Semana " . $row['week'] . " de " . $row['year'];
    $ganancias[] = $row['total_semana'];
}
...
<body>
    ...
    <div class="navbar_container">
        ...
        <nav class="navbar_menu" id="navbarMenu">
            ...
            <a href="ganancias.php" class="navbar_link">Ganancias Diarias</a>
            <a href="gananciassemanales.php" class="navbar_link active">Semanal</a>
    ...
    <div class="filters">
        ...
        <button onclick="window.location.href='ganancias.php';" class="filter_button">Diario</button>
        <button class="filter_button active" data-filter="all">Semanal</button>
